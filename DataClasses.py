...
# DBiesinger, 2022-Jan-02, Modified to add Track class, added methods to CD class to handle tracks
...
class Track():
    ...
    def __str__(self):
        ...
        return '{:>2}\t{} Time:{}'.format(self.position, self.title, self.length)
...
class CD:
    ...
    def add_track(self, track: Track) -> None:
        ...
        if n >= track.position:
            self.__tracks[track.position - 1] = track
        else:
            self.__tracks += [None] * (track.position - n)
            self.__tracks[track.position - 1] = track
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        try:
            for i in range(len(self.__tracks)):
                if self.__tracks[i] is not None and self.__tracks[i].position == track_id:
                    del self.__tracks[i]
        except Exception:
            raise Exception('Please input a valid track id!')
